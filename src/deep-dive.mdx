...
Function mode allows you to create nested modals. Spawning a modal inside a modal with `cb-react-modal` is a piece of cake. As you don't need to worry about managing states it is extremely practical to use function mode to spawn nested modals.
...
```jsx
import { closeModal } from 'cb-react-modal';
```
